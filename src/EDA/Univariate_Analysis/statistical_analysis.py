...
from src.EDA.Univariate_Analysis.common import (
    column_metadata,
    detect_column_type,
    safe_float,
)
...
def analyze_numeric(s: pd.Series) -> dict[str, Any]:
  ...
  # Safeguard: Convert boolean columns to integers (1 and 0) to avoid numpy subtraction errors
...
def analyze_categorical(s: pd.Series, top_n: int = 10) -> dict[str, Any]:
  ...
  result = {
      ...
      "descriptive": {
          ...
          (
              "mode_percentage"
          ): float(counts.iloc[0] / len(x) * 100) if len(x) else 0,
      },
      "frequencies": [
          {
              "category": str(cat),
...
def analyze_datetime(s: pd.Series) -> dict[str, Any]:
  ...
  result = {
      ...
      "frequency": {
          "year": {
              str(k): int(v)
              for k, v in x.dt.year.value_counts().sort_index().items()
          },
          "month": {
              str(k): int(v)
              for k, v in x.dt.month.value_counts().sort_index().items()
          },
          "day_of_week": {
              str(k): int(v)
              for k, v in x.dt.dayofweek.value_counts().sort_index().items()
          },
      },
  }
...
def save_analysis(
    ...
    filename: Path = BASE_DIR/ "src"/ "EDA"/ "Univariate_Analysis"/ "Reports"/ "statistical_analysis.json",) -> None:
...
def perform_univariate_statistical_analysis(file_path: Path) -> dict[str, Any]:

  """Perform univariate statistical analysis on a DataFrame and return the results."""
...
if __name__ == "__main__":
  perform_univariate_statistical_analysis(file_path=BASE_DIR / "src" / "Data" / "ecommerce_sales.csv")