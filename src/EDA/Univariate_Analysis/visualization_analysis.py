...
def numeric_visualizations(column: str, bins: int = 30) -> list[dict[str, Any]]:
  ...
  return [
      ...
      },
      {
          "id": f"{column}_kde",
          "type": "kde",
          "purpose": "distribution_shape",
          "x": column,
      },
      {
          "id": f"{column}_qq",
          "type": "qq_plot",
          "purpose": "normality_assessment",
          "x": column,
      },
  ]
...
def datetime_visualizations(column: str) -> list[dict[str, Any]]:
  ...
  return [
      {
          "id": f"{column}_timeline",
          "type": "line",
          "purpose": "time_distribution",
          "x": column,
      },
      {
          "id": f"{column}_year",
          "type": "bar",
          "purpose": "year_frequency",
          "x": column,
          "time_unit": "year",
      },
      {
          "id": f"{column}_month",
          "type": "bar",
          "purpose": "month_frequency",
          "x": column,
          "time_unit": "month",
      },
      {
          "id": f"{column}_weekday",
          "type": "bar",
          "purpose": "weekday_frequency",
          "x": column,
...
def save_visualization_analysis(
    analysis: dict[str, Any],
    filename: Path = BASE_DIR
    / "src"
    / "EDA"
    / "Univariate_Analysis"
    / "Reports"
    / "visualization_analysis.json",
) -> None:
...
def render_numeric(df: pd.DataFrame, column: str, bins: int = 30) -> plt.Figure:
  ...
  s = df[column]
  # Safeguard: Convert boolean series to integers to avoid numpy/matplotlib boolean subtraction/histogram errors
...
def render_visualizations(
    df: pd.DataFrame, output_dir: Path | str = "plots", bins: int = 30
) -> None:
  ...
  for column in df.columns:
    ...
    fig.savefig(
        output / f"{column}_visualization.png", dpi=150, bbox_inches="tight"
    )
    plt.close(fig)
...
def perform_univariate_visualization_analysis(file_path: Path) -> None:
  """Perform univariate visualization analysis on a CSV file and save results."""
...
if __name__ == "__main__":
  perform_univariate_visualization_analysis(file_path=BASE_DIR / "src" / "Data" / "ecommerce_sales.csv")